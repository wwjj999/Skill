...
foreach ($dockerfile in $dockerfiles) {
    ...
    if ($useDocker) {
        ...
        $dockerArgs = @("run", "--rm", "-i", "-v", "${dockerfileDir}:/workspace")

        if ($ConfigFile) {
            $resolvedConfig = (Resolve-Path $ConfigFile).Path
            $configDir = Split-Path $resolvedConfig
            $configName = Split-Path $resolvedConfig -Leaf
            $dockerArgs += "-v", "${configDir}:/config", "hadolint/hadolint", "--config", "/config/$configName", "/workspace/$dockerfileName"
        }
        else {
            $dockerArgs += "hadolint/hadolint", "/workspace/$dockerfileName"
        }

        $dockerInput = Get-Content -Path $dockerfile -Raw
        $result = $dockerInput | & docker @dockerArgs 2>&1
    }
    else {
        $args = @($dockerfile)
        if ($ConfigFile) {
