...
if [[ "$PATH_ABSOLUTE" == *"/.agents"* || "$PATH_ABSOLUTE" == *"/bmad"* ]]; then
    echo -e "${RED}❌ 安全策略: 禁止在受保护目录运行 ai-agent-lint${NC}"
    echo -e "${YELLOW}   Protected: .agents/ and bmad/${NC}"
    exit 1
fi
...
# 构建 Ruff 命令（使用数组，禁止 eval）
RUFF_CMD=("ruff" "check" "$PATH_ABSOLUTE" "--exclude" ".agents" "--exclude" "bmad")
