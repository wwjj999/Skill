...
if (-not $AllowProtected -and ($targetPathString -match '[\\/]\.agents([\\/]|$)' -or $targetPathString -match '[\\/]bmad([\\/]|$)')) {
    Write-Host "❌ 安全策略: 禁止在受保护目录运行 ai-agent-lint" -ForegroundColor Red
    Write-Host "   Protected: .agents/ and bmad/" -ForegroundColor Yellow
    exit 1
}
...
if (-not $AllowProtected) {
    $ruffArgs += "--exclude", ".agents", "--exclude", "bmad"
}
if ($Fix) {
    $ruffArgs += "--fix"
