...
class SkillRunner:
    ...
    def _print(self, message: str, color: str = 'RESET'):
        ...
        if sys.platform == 'win32':
            # 某些终端（如 GitHub Actions, VSCode）自动处理 ANSI
            if os.getenv('TERM') in ['xterm', 'xterm-256color'] or os.getenv('VSCODE_PID'):
                pass
            else:
                try:
                    import ctypes
                    kernel32 = ctypes.windll.kernel32
                    # 尝试开启 ENABLE_VIRTUAL_TERMINAL_PROCESSING (0x0004)
                    handle = kernel32.GetStdHandle(-11)
                    mode = ctypes.c_uint()
                    kernel32.GetConsoleMode(handle, ctypes.byref(mode))
                    kernel32.SetConsoleMode(handle, mode.value | 0x0004)
                except Exception:
                    # 如果开启失败且不是现代终端，回退到无颜色输出
                    print(message)
                    return
    ...
    def run(self, path: str = '.', check_mode: bool = False, 
        ...
        if not os.path.exists(safe_path):
            self._print(f"❌ 错误: 路径不存在 - {safe_path}", 'RED')
            return 1
        ...
        for part in self.tool_command:
            if '{path}' in part:
                cmd.append(part.replace('{path}', safe_path))
            else:
                cmd.append(part)
