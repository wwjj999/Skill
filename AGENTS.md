...
## 🧬 Structured Governance (Evo-Asset Inspired)

To reduce ambiguity, governance follows deterministic quality gates:

1. **Single-Path Boot Gate**: Boot always executes first; no optional branch.
2. **Dialogue Strategy Gate**: Complete user interview/alignment before any code generation.
3. **Protected Land Gate**: `.agents/` and `bmad/` are read-only unless explicit maintenance authorization exists.
4. **Security Gate**: Dynamic execution (`eval`, `Invoke-Expression`, shell interpolation execution) is forbidden by default.
5. **Traceability Gate**: Every protocol-affecting change must be auditable via structured changelog entries.

### 📋 Active Governance Rules

1. **🛑 COGNITIVE MIRRORING PROTOCOL (Language Prime Directive)**:
...
> *Rules 0–8 above govern Active AI Behavior. The Structured Governance Gates (§ above) govern Boot/Security flow. Both are mandatory.*
...
- **evo-asset** - Structure and validate Gene/Capsule/EvolutionEvent assets
  - Location: `.agents/skills/evo-asset/SKILL.md`
  - Commands: `node .agents/skills/evo-asset/scripts/compute_asset_id.js <asset.json>` or `python .agents/skills/evo-asset/scripts/compute_asset_id.py <asset.json>`
...
### 🔴 Protected Land (`.agents/*`, `bmad/*`)

- **Status**: **IMMUTABLE** (Read-Only).
- **Rule**: You are **FORBIDDEN** from deleting, moving, or modifying these files (unless in Protocol Maintenance Mode or with explicit user maintenance authorization).

### 🟢 User Land (Any Path outside `.agents/` and `bmad/`)

- **Status**: **READ-WRITE**.
- **Rule**: You have **FULL AUTHORITY** to create, edit, refactor, and delete code in any path **outside** of `.agents/` and `bmad/` (including the project root and `sandbox/`).
...
> 2. **EXECUTE**: Start [BOOT_SEQUENCE] immediately. Boot cannot be skipped.
> 3. **DIALOGUE-FIRST**: Complete `PERSONA_HANDSHAKE` and strategy alignment before any code generation.
...
**Read-Only Override**: In this project, `bmad/` is Protected Land. Run detection/fallback only; do not auto-install, rename, or mutate BMAD runtime unless explicitly authorized for maintenance.
...
4. **MEMORY_GUARDIAN_CHECK**:
    - **Trigger**: Read `.agents/skills/memory-guardian/SKILL.md`.
    - **Action**: Check if the system memory is being monitored.
        1. Verify if the `memory-guardian` daemon is running in the background.
        2. If NOT running, silently execute the cross-platform daemon startup command: `python .agents/skills/memory-guardian/scripts/monitor.py --daemon` (use `python3` or `python` depending on the OS, or `start /B` on Windows / `nohup` on Linux/macOS to ensure it detaches).
        3. **CRITICAL**: The daemon MUST be started in the background so it does not block the AI's current execution thread or the user's terminal.
