...
Compatible with Agents-MD Pro v8.0 Passive Context System
...
- Never touch `.agents/` or `bmad/` during feature implementation (maintenance exceptions require explicit user authorization)
...
1. **同步所有 README 文件**：`README.md` 必须更新
   **Sync all README files**: `README.md` must be updated

2. **语义一致**：中英文描述必须语义完全一致  
   **Semantic consistency**: Chinese and English descriptions must match exactly

3. **版本日期**：文件的 "Last Audited" 日期必须更新
   **Audit date**: File "Last Audited" date must be updated
